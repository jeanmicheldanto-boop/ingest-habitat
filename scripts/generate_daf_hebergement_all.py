#!/usr/bin/env python3
"""Génère prospection-financeurs/data/output/DAF_HEBERGEMENT_SOCIAL_ALL_100.csv
Filtre: gestionnaires avec nb_etablissements>10 et secteur_activite_principal ILIKE '%heberg%'.
Ne requiert pas d'email ni fonction financière (inclus tous les gestionnaires matching).
"""
...
WHERE g.nb_etablissements > 10
    AND (
        COALESCE(g.secteur_activite_principal, '') ILIKE '%héberg%'
        OR COALESCE(g.secteur_activite_principal, '') ILIKE '%heberg%'
    )
...
def main():
    ...
    out_file = out_dir / "DAF_HEBERGEMENT_SOCIAL_ALL_100.csv"
