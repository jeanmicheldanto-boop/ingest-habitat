#!/usr/bin/env python3
"""Génère prospection-financeurs/data/output/DAF_30.csv en combinant daf_* et dirigeants finance."""
...
WHERE g.nb_etablissements > 10
  AND (
    COALESCE(g.secteur_activite_principal, '') ILIKE '%handicap%'
    OR COALESCE(g.secteur_activite_principal, '') ILIKE '%protection%'
    OR COALESCE(g.secteur_activite_principal, '') ILIKE '%enfant%'
  )
  AND (
...
def main():
    ...
    try:
        ...
        with out_file.open('w', encoding='utf-8', newline='') as fh:
            ...
            writer.writerow(['id_gestionnaire','raison_sociale','prenom','nom','email','poste'])
            for r in rows:
                writer.writerow(r)
