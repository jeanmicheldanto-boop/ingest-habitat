...
def step_3_geocoding():
    ...
    if st.button("🎯 Démarrer le géocodage", type="primary"):
        ...
        geocoding_svc = GeocodingService(google_api_key if geocoding_service == "Google Maps (API key requise)" else None)
        ...
        for i, (idx, row) in enumerate(importable_df.iterrows()):
            ...
            if address:
                ...
                # Géocoder l'adresse
                if geocoding_service == "Google Maps (API key requise)":
                    result = geocoding_svc.geocode_google(address)
                else:
                    result = geocoding_svc.geocode_nominatim(address)
                ...
                if result:
                    geocoding_results.append({
                        ...
                        'geocode_precision': result.get('precision', 'Unknown')
                    })
                else:
                    geocoding_results.append({
                        'index': idx,
...
def step_4_enrichment():
    ...
    with col1:
        enrich_contact = st.checkbox("📞 Enrichir les informations de contact", value=True)
        enrich_description = st.checkbox("📝 Récupérer les descriptions", value=True)
    
    with col2:
        enrich_services = st.checkbox("🛎️ Détecter les services", value=True)
        enrich_images = st.checkbox("🖼️ Récupérer les images", value=False, help="Expérimental")
    
    # Sélection des établissements à enrichir
    st.subheader("🎯 Sélection des établissements")
    
    # Identifier les établissements avec site web
    establishments_with_websites = []
    
    if mapping.get('site_web'):
        site_col = mapping['site_web']
        for idx, row in importable_df.iterrows():
            site_web = row[site_col]
            if pd.notna(site_web) and site_web.strip() and site_web.startswith('http'):
                nom = row[mapping.get('nom', importable_df.columns[0])] if mapping.get('nom') else f"Ligne {idx + 1}"
                establishments_with_websites.append({
                    'index': idx,
                    'nom': nom,
                    'site_web': site_web
                })
    
    if not establishments_with_websites:
        st.warning("⚠️ Aucun établissement avec site web détecté.")
        st.write("Assurez-vous que votre fichier contient une colonne 'site_web' ou équivalent avec des URLs valides.")
    else:
        st.write(f"📊 {len(establishments_with_websites)} établissements avec site web détectés.")
        
        # Affichage des établissements
        websites_df = pd.DataFrame(establishments_with_websites)
        st.dataframe(websites_df, use_container_width=True)
        
        # Sélection des établissements à traiter
        max_establishments = min(10, len(establishments_with_websites))
        num_to_process = st.slider(
            "Nombre d'établissements à traiter:",
            1, len(establishments_with_websites), max_establishments,
            help="Limiter le nombre pour éviter les timeouts"
        )
        
        # Démarrer l'enrichissement
        if st.button("🚀 Démarrer l'enrichissement", type="primary"):
            progress_bar = st.progress(0)
            status_text = st.empty()
            
            enrichment_results = []
            
            for i, establishment in enumerate(establishments_with_websites[:num_to_process]):
                status_text.text(f"Enrichissement en cours... {establishment['nom']}")
                progress_bar.progress((i + 1) / num_to_process)
                
                # Enrichir depuis le site web
                enrichment_data = web_enrichment_service.enrich_from_website(establishment['site_web'])
                enrichment_data['establishment_index'] = establishment['index']
                enrichment_data['establishment_nom'] = establishment['nom']
    ...
    if st.session_state.enrichment_results:
        ...
        with col3:
            success_rate = len(successful_enrichments) / len(st.session_state.enrichment_results) * 100
            st.metric("📈 Taux de réussite", f"{success_rate:.1f}%")
        
        # Détails par établissement
        for result in st.session_state.enrichment_results:
            with st.expander(f"🏢 {result['establishment_nom']}"):
                if 'error' in result:
                    ...
                    if result.get('description'):
                        st.write(f"**Description:** {result['description'][:200]}...")
                    
                    if result.get('contact_info'):
                        contact = result['contact_info']
                        if contact.get('emails'):
                            st.write(f"**Emails:** {', '.join(contact['emails'][:3])}")
                        if contact.get('phones'):
                            st.write(f"**Téléphones:** {', '.join(contact['phones'][:3])}")
    ...
    if st.button("🎯 LANCER L'IMPORT EN BASE", type="primary"):
        ...
        try:
            for i, validation_record in enumerate(records_to_import):
                ...
                # Import en base
                result = st.session_state.database_manager.insert_establishment(import_data)
