...
class ExtractedEstablishment:
    ...
    commune: str
    code_postal: str
    gestionnaire: str  # AJOUTÉ pour compatibilité
    adresse_l1: str    # RENOMMÉ de adresse
    telephone: Optional[str]
    email: Optional[str]
    site_web: Optional[str]
    sous_categories: str  # AJOUTÉ pour compatibilité
    habitat_type: str     # RENOMMÉ de type_habitat  
    eligibilite_avp: str  # AJOUTÉ pour compatibilité
    presentation: str     # RENOMMÉ de description
    departement: str
    source: str          # RENOMMÉ de source_url
    date_extraction: str # AJOUTÉ pour compatibilité
    public_cible: str
    ...
    def _stage3_heavy_extraction(self, candidates: List, department: str) -> List[ExtractedEstablishment]:
        ...
        for i, candidate in enumerate(candidates, 1):
            ...
            if response and response.get("establishments"):
                ...
                # Créer objets ExtractedEstablishment - FORMAT CSV COMPATIBLE
                for est_data in establishments:
                    establishment = ExtractedEstablishment(
                        ...
                        adresse_l1=est_data.get("Adresse", est_data.get("adresse", "")),  # CORRIGÉ
                        ...
                        sous_categories=est_data.get("Type", est_data.get("sous_categories", "")),
                        habitat_type=est_data.get("Type", est_data.get("habitat_type", "")),
...
Si aucun établissement: {{"establishments": [], "confidence": 0}}"""
    ...
    def _call_heavy_llm(self, prompt: str) -> Optional[Dict]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Parse JSON response
                import json
                print(f"📝 Réponse Groq HEAVY brute: '{content[:300]}'...")
                
                # Nettoyer le préambule
                cleaned_content = content
                if '{' in content:
                    json_start = content.find('{')
                    cleaned_content = content[json_start:]
                
                print(f"📝 Réponse nettoyée: '{cleaned_content[:200]}'...")
            ...
            print(f"📝 Détail erreur: {type(e).__name__}")
            if hasattr(e, 'response'):
                print(f"📡 Réponse: {e.response.text}")
            # SIMULATION DÉSACTIVÉE - Retourner None en cas d'erreur
            return None
            import random
            time.sleep(1.5)
    ...
    def _print_quality_summary(self, initial_candidates: List, final_establishments: List[ExtractedEstablishment], department: str):
        ...
        for est in final_establishments:
            type_counts[est.habitat_type] = type_counts.get(est.habitat_type, 0) + 1
    ...
    def export_to_csv(self, establishments: List[ExtractedEstablishment], filename: str):
        """Exporte les établissements au format CSV"""
        ...
        with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            for est in establishments:
                writer.writerow({
                    ...
                    "gestionnaire": getattr(est, 'gestionnaire', ''),
                    "adresse_l1": getattr(est, 'adresse_l1', est.adresse if hasattr(est, 'adresse') else ''),
                    "telephone": est.telephone or "",
                    "email": est.email or "",
                    "site_web": est.site_web,
                    "sous_categories": getattr(est, 'sous_categories', est.type_habitat if hasattr(est, 'type_habitat') else ''),
                    "habitat_type": getattr(est, 'habitat_type', est.type_habitat if hasattr(est, 'type_habitat') else ''),
                    "eligibilite_avp": getattr(est, 'eligibilite_avp', 'a_verifier'),
                    "presentation": getattr(est, 'presentation', est.description if hasattr(est, 'description') else ''),
                    "departement": est.departement,
                    "source": getattr(est, 'source', est.source_url if hasattr(est, 'source_url') else ''),
                    "date_extraction": getattr(est, 'date_extraction', est.validation_timestamp),
